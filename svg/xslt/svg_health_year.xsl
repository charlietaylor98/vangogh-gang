...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <svg height="{$max_height + 200}" width="{$max_width + 200}">
            <g transform="translate({$half_width + 50}, 100)">
                ...
                <text x="0" y="{$max_height + 50}" font-size="18" text-anchor="middle"
                    font-weight="300">
                    <xsl:text>Mentions of Mental Health by Year: Good vs. Bad</xsl:text>
                </text>
                ...
                <xsl:apply-templates select="letters/year" mode="good"/>
                <xsl:apply-templates select="letters/year" mode="bad"/>
            </g>
        </svg>
    </xsl:template>
    <xsl:template match="year" mode="good">
    ...
    <xsl:template match="year" mode="bad">
        <xsl:variable name="length" as="xs:double"
