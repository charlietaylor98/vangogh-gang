        ...
        <svg height="{$max_height + 200}" width="{$max_width + 250}">
            <g transform="translate({$half_width + 50}, 100)">
                ...
                <text x="{$half_width div 2}" y="-50" font-size="16" text-anchor="middle"
                    font-weight="300">
                    <xsl:text>Count: Good Health</xsl:text>
                </text>
                <text x="-{$half_width div 2}" y="-50" font-size="16" text-anchor="middle"
                    font-weight="300">
                    <xsl:text>Count: Bad Health</xsl:text>
                </text>
                <text x="0" y="{$max_height + 50}" font-size="18" text-anchor="middle"
                    font-weight="300">
                    <xsl:text>Mentions of Mental Health by Location: Good vs. Bad</xsl:text>
                </text>
                ...
                <xsl:apply-templates select="letters/location" mode="good"/>
                <xsl:apply-templates select="letters/location" mode="bad"/>
            </g>
        </svg>
    </xsl:template>
    <xsl:template match="location" mode="good">
        <xsl:variable name="length" as="xs:double"
            select="sum(descendant::good_health/count(.))"/>
        ...
        <xsl:for-each select=".">
            ...
            <text x="{$half_width + 25}" y="{$ypos + $spacing}" text-anchor="start" font-size="12">
                <xsl:value-of select="@id/translate(., '_', ' ')"/>
            </text>
        </xsl:for-each>
    </xsl:template>
    <xsl:template match="location" mode="bad">
        <xsl:variable name="length" as="xs:double"
            select="sum(descendant::bad_health/count(.))"/>
        <xsl:variable name="ypos" as="xs:double"
            select="$spacing + (position() - 1) * ($bar_height + $spacing)"/>
        <xsl:for-each select=".">
